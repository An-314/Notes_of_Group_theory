...
对于给定的杨图$[lambda]$，所有生成元$(k k-1)(k = 2,3,...,n)$的不可约酉表示的矩阵$U^[lambda](k-1,k)$可以利用如下规律构造出来：
- 若数字$k - 1$和$k$出现在标准杨盘$T^[lambda]$的同一行时，则有
  $
    U^[lambda]_(r r) (k-1, k) = 1
  $
- 若数字$k - 1$和$k$出现在标准杨盘$T^[lambda]$的同一列时，则有
  $
    U^[lambda]_(r r) (k-1, k) = -1
  $
- 若数字$k - 1$和$k$不在标准杨盘$T^[lambda]$的同一行或同一列时，且有
  $
  ...
  $
  其中$rho^(-1)$是杨盘中数 $k - 1$到$k$的*轴距离*。在杨盘中，数字$a$到$b$的轴距离定义为：从$a$开始数格子一直到$b$，向左或向下数一个格子为1，向右或向上数一个格子为$-1$，这样数出的代数和称为$a$到$b$的轴距离。
  #figure(
    image("pic/2025-06-10-13-22-59.png", width: 30%),
    caption: [杨盘中数字的轴距离, 图中$a$到$b$的轴距离为4],
  )
- 其他情况
  $
